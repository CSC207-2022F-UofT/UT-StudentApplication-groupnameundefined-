...
import static frontend.util.Calendar.roundTime;
	...
	public void updateCal(TimetableSchema timetable, UserSchema userSchema, WebClient webClient) {
		...
		for (BlockSchema block : timetable.getBlocks()) {
			...
			if (block.getType().equals("SEC")) {
				events.add(new CalendarEvent(block.getStartDay(), block.getStartMil(), block.getEndMil(), block.getSection()
						.map(x -> x.getCourse().getName())
						.orElse("Misc Course"), Color.PINK));
			} else if (block.getType().equals("APT")) {
				events.add(new CalendarEvent(block.getStartDay(), block.getStartMil(), block.getEndMil(), block.getLocation()
						.orElse("Misc Location"), Color.CYAN));
			} else {
		...
		cal.addCalendarEmptyClickListener(e -> {
			...
			response.subscribe(r -> {
				...
				jcbStartTime.setSelectedItem(roundTime(e.getDateTime().toLocalTime(), 30)
						.toString());
				...
				jcbEndTime.setSelectedItem(roundTime(e.getDateTime().toLocalTime().plusHours(1), 30)
						.toString());
